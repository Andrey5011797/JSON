...
function createProfileObject() {
  const newProfile = {
    id: idCounter++,
    userName: userNameField.value.trim() || "Без имени",
...
function doFilteredExport() {
  ...
  const selectedKeys = Array.from(keyCheckboxes)
...
function importFromJson() {
  try {
    ...
    if (parsedObject.createdAt && typeof parsedObject.createdAt === "Строка") {
      parsedObject.createdAt = new Date(parsedObject.createdAt);
    }
    if (parsedObject.createdAt && isNaN(parsedObject.createdAt.getTime())) {
    ...
    errorContainer.style.display = "ничто";
  } catch (error) {
    errorDescription.textContent = `Недопустимый JSON: ${error.message}`;
    errorContainer.style.display = "Блок";
  }
}
